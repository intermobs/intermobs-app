...
import { Eye, EyeOff, Lock, LogIn, Mail, Orbit, Sparkles } from 'lucide-react';
          ...
          <div className="relative">
            <div className="mb-8 text-center">
              <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-cyan-400/20 bg-cyan-400/10">
                <Sparkles size={25} className="text-cyan-300" />
              </div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Sign in to continue into Intermobs.
              </p>
            </div>

            {error && (
              <div className="mb-5 rounded-xl border border-red-400/20 bg-red-400/10 p-3 text-center text-sm font-medium text-red-300">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="group mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-white py-3.5 font-semibold text-black shadow-lg transition duration-300 hover:bg-cyan-300 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {isLoading ? (
                  'Signing in...'
                ) : (
